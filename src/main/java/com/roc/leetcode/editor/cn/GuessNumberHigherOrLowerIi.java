...
public class GuessNumberHigherOrLowerIi {
    ...
    class Solution {
        ...
        private int getMoneyAmount(int start, int end, int[][] dp) {
            ...
            int min = Math.min(start + getMoneyAmount(start + 1, end, dp), getMoneyAmount(start, end - 1, dp) + end);
            for (int i = start + 1; i < end; i++) {
                min = Math.min(min, Math.max(getMoneyAmount(start, i - 1, dp), getMoneyAmount(i + 1, end, dp)) + i);
            }
            dp[start][end] = min;
            return min;
        }
