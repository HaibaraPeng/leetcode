...
// Related Topics 树 二叉搜索树 二叉树 👍 762 👎 0
    ...
    class Solution {
        public TreeNode deleteNode(TreeNode root, int key) {
            if (root == null) {
                ...
                return root.left;
            } else if (root.val == key && root.right.left == null) {
                ...
                tempPre.left = deleteNode(temp, temp.val);
                temp.left = root.left;
        ...
        private void deleteNode(TreeNode preNode, TreeNode node, int key, boolean isLeft) {
            if (node == null) {
                return;
            }
            if (node.val > key) {
                ...
                if (isLeft) {
                    preNode.left = deleteNode(node, node.val);
                } else {
                    preNode.right = deleteNode(node, node.val);
                }
            }
        }
    }
